...
@pytest.mark.smoke
@pytest.mark.asyncio
async def test_github_oauth_start_matches_auth_shell_readiness_without_secret_leakage() -> None:
    require_smoke()
    ...
    assert readiness["components"]["oauth_github"] in {"ready", "unconfigured", "unavailable"}
    assert_oauth_start_contract(response, provider="github")
