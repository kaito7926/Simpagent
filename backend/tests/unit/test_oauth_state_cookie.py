...
from app.security.oauth_transaction import issue_oauth_transaction, seal_oauth_transaction, unseal_oauth_transaction
...
def test_oauth_transaction_preserves_dpop_thumbprint_binding(settings) -> None:
    transaction = issue_oauth_transaction(
        provider="google",
        settings=settings,
        now=settings.now_utc(),
        dpop_key_thumbprint="A" * 43,
    )

    restored = unseal_oauth_transaction(
        cookie_value=seal_oauth_transaction(transaction=transaction, settings=settings),
        provider="google",
    ...
    assert restored.dpop_key_thumbprint == "A" * 43
