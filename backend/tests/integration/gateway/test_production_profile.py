...
def _base_settings(**overrides):
    values = {
        ...
        "message_encryption_key_file": "/run/secrets/message_encryption_key",
...
def _service_block(contents: str, service_name: str) -> str:
    ...
    assert start is not None, f"service {service_name} missing from compose.yaml"
...
def test_env_example_documents_small_production_profile_without_real_secrets() -> None:
    ...
    required_keys = {
        ...
        "SIMPAGENT_MESSAGE_ENCRYPTION_KEY_FILE=/run/secrets/message_encryption_key",
...
def test_long_running_compose_services_restart_unless_stopped() -> None:
    contents = COMPOSE_FILE.read_text(encoding="utf-8")

    for service_name in (
        "postgres",
        "backend",
        "backend-small-production",
        "frontend",
        "kong",
        "loki",
        "tempo",
        ...
        block = _service_block(contents, service_name)
        assert "    restart: unless-stopped" in block

    for service_name in ("migrate", "init-dev-secrets", "seed-demo", "init-kong-logs"):
        block = _service_block(contents, service_name)
