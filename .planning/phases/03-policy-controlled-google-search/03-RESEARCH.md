...
## 2026-06-22 Addendum: Firecrawl Cloud Provider Option

The new requirement is not "replace Gemini," but "generalize the websearch boundary so Gemini and Firecrawl are both first-class providers behind the same policy-controlled tool." The safest architecture is to keep the existing public search contract (`tool:websearch`, one-tool-per-turn, persisted `SearchResponseState`, correlated `ToolExecution`) and insert a provider-selection layer behind it. The browser and ordinary chat route should still request only `google_search`/websearch mode; the backend then resolves the effective provider from an allowlist, checks readiness for that provider, and dispatches to the matching internal worker/client. This preserves AUTHZ-04, AUTHZ-07, AGNT-01, and AGNT-03 while avoiding a second public tool surface. [VERIFIED: backend/app/services/chat_turns.py; backend/app/agent/coordinator.py; backend/app/schemas/search.py]

The current implementation already has the right seams for this upgrade. `Settings` owns search credentials and timeouts; `search_status()` already centralizes readiness; `GoogleSearchWorkerService` already normalizes the provider result into `SearchWorkerResult`; the admin/orchestration path already persists runtime safety toggles through `AgentRuntimeSettingsRepository`, `AdminEvidenceService`, `OrchestrationSettingsResponse`, and `SettingsPage`. The right planning move is therefore an incremental enhancement, not a full Phase 3 replan: add provider-selection config and runtime override support, add a Firecrawl Cloud client that returns the same normalized `SearchWorkerResult` envelope, and extend the admin orchestration contract so admins can view/change the effective provider without ever handling raw secrets in the dashboard. [VERIFIED: backend/app/core/config.py; backend/app/core/provider_status.py; backend/app/ai/search_worker/service.py; backend/app/db/repositories/agent_settings.py; backend/app/services/admin_evidence.py; frontend/components/settings/SettingsPage.tsx]

Firecrawl introduces two planning implications that Gemini-only research did not cover. First, the backend must treat Firecrawl as an API-key-backed external search provider rather than an ADK grounding worker; the planning contract should therefore assume a separate typed client or service boundary such as `FirecrawlSearchClient.search()` with explicit request/output budgets and no arbitrary URL fetch capability. Second, the user-facing search UX can no longer hardcode `Google-grounded`, `Tìm bằng Google`, or Google-only suggestion semantics as universal truths. The safe product contract is provider-honest and provider-neutral by default: keep the same grounded/missing-grounding/denied/unavailable/provider-failed/timeout state matrix, but render Google-specific badges or Search Suggestions only when the selected provider actually returns those trusted fields. [VERIFIED: .planning/phases/03-policy-controlled-google-search/03-UI-SPEC.md; frontend/lib/chat-session.ts; frontend/components/chat/ConversationHeader.tsx; frontend/components/settings/SettingsPage.tsx]

Configuration precedence should be explicit in the plans so execution agents do not invent it ad hoc. Recommended order: admin runtime override if present, otherwise environment default, otherwise the existing safe default. Concrete config targets should include an allowlisted provider setting such as `SIMPAGENT_WEBSEARCH_PROVIDER=gemini|firecrawl`, Firecrawl credentials via `SIMPAGENT_FIRECRAWL_API_KEY` or `FIRECRAWL_API_KEY` plus optional file-backed secret loading, and provider-status reporting that distinguishes `unconfigured`, `ready`, and provider-specific unavailable/unsupported states. When Firecrawl is selected without a key, or when Gemini is selected with an unsupported model, the backend must return `search_unavailable` rather than silently fail over to the other provider. [VERIFIED: backend/app/core/config.py; backend/app/core/provider_status.py; backend/tests/integration/search/test_search_capability_check.py]

This new scope also creates a terminology drift that planning should address directly. ROADMAP and Phase 3 artifacts still describe the slice as `Policy-Controlled Google Search`, and the shipped UI/tests currently encode Google-specific copy. Because the user is explicitly expanding the feature to Firecrawl, the next executable plans should include targeted doc/UI generalization work: retain the existing requirement IDs for continuity, but update the implementation-facing contract toward `policy-controlled websearch` where provider-neutral behavior is required. That is best handled as two incremental plans added after `03-04`: one backend/admin-runtime plan for provider selection and Firecrawl integration, and one frontend/verification plan for provider-honest UX, admin dashboard control, and dual-provider regression coverage. [VERIFIED: .planning/ROADMAP.md; .planning/phases/03-policy-controlled-google-search/03-01-PLAN.md; .planning/phases/03-policy-controlled-google-search/03-02-PLAN.md; .planning/phases/03-policy-controlled-google-search/03-03-PLAN.md; .planning/phases/03-policy-controlled-google-search/03-04-PLAN.md]
